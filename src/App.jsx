...
import{getBestMoveMCTS}from'./mcts.js';
...
const isAITurn=!gameOver&&!isProcessingPostGame&&((mode==='pve_first'&&turn==='black')||(mode==='pve_second'&&turn==='red'));
const resetGame=()=>{
...
className={`w-10 h-10 flex flex-col items-center justify-center rounded border-2 ${count===0?disabledColor:chipColor} ${isSelected?'border-yellow-400 ring-2 ring-yellow-400':'border-transparent'}`}
>
...
className={`w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded cursor-pointer transition-colors ${cell?(cell.player==='red'?'bg-red-600 text-white':'bg-gray-800 text-white'):'bg-white hover:bg-gray-200'}`}
>
